...
from .models import SeaRouteModel, RailRouteModel
...
async def find_all_paths(
    ...
    query_rail = (
        ...
        .where(
            ...
            joinedload(rail.start_point), joinedload(rail.end_point),
            joinedload(rail.company), joinedload(rail.container),
        )
    )
    query_sea = (
        select(sea)
        .where(
            ...
            joinedload(sea.start_point), joinedload(sea.end_point),
            joinedload(sea.company), joinedload(sea.container),
        )
    )
    query_sea_rail = (
        select(sea, rail)
        ...
        .join(rail, (sea.end_point_id == rail.start_point_id) & (sea.container_id == rail.container_id))
        .options(
            joinedload(sea.start_point), joinedload(sea.end_point),
            joinedload(sea.company), joinedload(sea.container),
    ...
    query_rail_sea = (
        ...
        .where(
            ...
            & rail.container_id.in_(container_ids))
        .join(sea, (rail.end_point_id == sea.start_point_id) & (rail.container_id == sea.container_id))
        .options(
            joinedload(rail.start_point), joinedload(rail.end_point),
            joinedload(rail.company), joinedload(rail.container),
    ...
    query_rail_sea_rail = (
        ...
        .where(
            ...
            & rail.container_id.in_(container_ids))
        .join(sea, (rail.end_point_id == sea.start_point_id) & (rail.container_id == sea.container_id))
        .join(rail2, (sea.end_point_id == rail2.start_point_id) & (sea.container_id == rail2.container_id))
        .options(
            joinedload(rail.start_point), joinedload(rail.end_point),
            joinedload(rail.company), joinedload(rail.container),
            ...
            joinedload(rail2.start_point), joinedload(rail2.end_point),
            joinedload(rail2.company), joinedload(rail2.container),
        )
    )
