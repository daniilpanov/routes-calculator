...
def _map_segment(route, price, currency, _type, services, begin_cond = None, finish_cond = None):
    item = {
        ...
        'currency': currency,
        'services': {k: {'price': getattr(route, k), 'currency': c} for k, c in services.items()},
    }
...
def _map_route(routes):
    ...
    for route in routes:
        if getattr(route, 'filo', None):
            ...
            _types.append('rail')
            res.append(_map_segment(route, 'price', 'RUB', 'rail', {
                'guard': 'RUB',
            }))
