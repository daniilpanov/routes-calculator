...
async def _add_data(main_dict, data):
    try:
        ...
        print(e)
        return e
    for full_name, service, _id in map(lambda x: (union_country_and_name(main_dict, x.get('country'), x.get('name')), x['company'], x['id']), items):
...
async def all_destination_by_date(date: datetime.date, departure_point_id: str):
    ...
    if 'FESCO' in departure_ids:
        await _add_data(prepared_to, fesco.get_destination_points_by_date(date, departure_ids.pop('FESCO')))
    # from CUSTOM
